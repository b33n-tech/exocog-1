...
const pushToStack2Btn = document.getElementById("pushToStack2");
...
pasteDownloadBtn.addEventListener("click", async ()=>{
  try{
    ...
    console.error(err); alert("❌ JSON invalide !");
  }
});
...
prompts.forEach(p=>{
  ...
  btn.addEventListener("click", ()=>{
    ...
    navigator.clipboard.writeText(combined).then(()=>{copiedMsg.style.display="block"; setTimeout(()=>copiedMsg.style.display="none",2000);});
...
pushToStack2Btn.addEventListener("click", ()=>{
  localStorage.setItem("llmJson", JSON.stringify(tasks));
